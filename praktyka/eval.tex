...
\begin{enumerate}
    ...
    \item \textbf{Radar Wydajności Mobilnej} (Rys. \ref{fig:radar-chart}) – wielowymiarowe zestawienie kluczowych metryk (GPT Score, Latencja, Rozmiar modelu oraz Efektywność energetyczna). Wszystkie osie zostały znormalizowane do zakresu $[0, 1]$, gdzie krawędź zewnętrzna oznacza wynik optymalny.
    \begin{itemize}
        \item \textbf{Powierzchnia wielokąta}: Im większa powierzchnia zajmowana przez dany model, tym bardziej "zbalansowany" jest on pod kątem wdrożenia mobilnego. "Złoty Model" z poprzedniej sekcji to ten, którego wielokąt jest najbliższy kształtowi koła o promieniu 1.
        \item \textbf{Interpretacja bez legendy}: Ze względu na dużą liczbę porównywanych modeli, wykres radarowy może być renderowany bez jawnej legendy. Analiza opiera się na barwach – kolory zbieżne (np. odcienie zieleni) reprezentują rodziny modeli o podobnej wielkości, co pozwala na wizualną ocenę, jak np. kwantyzacja przesuwa wierzchołek latencji bez drastycznej degradacji punktu GPT Score.
    \end{itemize}

    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.85\linewidth]{rysunki/eval/run_071_full/all_models_with_reference_2026-01-18_01-56-08_radar.png}
        \caption{Radar Wydajności Mobilnej (Referenced). Wizualizacja balansu między jakością odpowiedzi a wymaganiami sprzętowymi. Idealny model brzegowy dąży do objęcia jak największego obszaru wykresu.}
...
Poniżej przedstawiono dwa reprezentatywne przypadki:
\begin{itemize}
    \item \textbf{Przykład "Dobry" (Granite 3.2 2B, Rys. \ref{fig:timeline-granite})}: Model niemal idealnie realizuje \textbf{Scenariusz B}. Linia RAM utrzymuje się stabilnie poniżej progu 8GB, a SWAP wykazuje zerową lub minimalną tendencję wzrostową. Jest to wzorcowy profil dla aplikacji mobilnej.
    \item \textbf{Przykład "Zły" (Llama 3.2 3B, Rys. \ref{fig:timeline-llama})}: Ilustracja \textbf{Scenariusza A}. Już po 3. rundzie dialogu następuje gwałtowny wzrost SWAP, który wyprzedza fizyczną zajętość RAM. Mimo braku throttlingu na MacBooku, na iPhone 15/16 zjawisko to wywołałoby natychmiastowe ubicie procesu przez systemowy mechanizm \textit{Jetsam}.
\end{itemize}
